...
public abstract class AbstractFileCheckerTest {

    private static final Logger logger = Logger.getLogger(AbstractFileCheckerTest.class.getSimpleName());
    ...
    private void setupFileChecker() throws Exception {
        // accept all files
        fileChecker = getSpecificFileChecker(rootTestingFolder.normalize().toString(), (dir, name) -> true);
        mockFileListener = new MockFileListener();
        fileChecker.addFileListener(mockFileListener);
        fileChecker.start();
    }

    protected abstract FileChecker getSpecificFileChecker(String directoryToWatch, FilenameFilter filenameFilter) throws IOException;
    ...
    public void testDoesNotAcceptNullDir() throws IOException {
        FileChecker fileChecker = getSpecificFileChecker(null, (dir, name) -> true);
    }

    @Test(expected = IOException.class)
    public void testDoesNotAcceptNonDir() throws IOException {
        FileChecker fileChecker = getSpecificFileChecker("pom.xml", (dir, name) -> true);
    }
