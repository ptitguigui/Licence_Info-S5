...
public class AutomataUtils {
	...
	public static void addSingleton(String word, AutomatonBuilder a, String namePrefix) {
		...
		for(int i=0; i<word.length(); i++){
			a.addNewState(namePrefix+word.substring(0, i));
		}
		a.setInitial(namePrefix+"epsilon");
		for(int i=0; i<word.length()-1; i++){
			a.addTransition(namePrefix+word.substring(0, i),word.charAt(i), namePrefix+word.substring(0, i+1));
		}	
		a.setAccepting(namePrefix+word);
	}
